...
URLs ── web_scraper.scrape_urls ──► list[Source]
        ...
        llm_summarizer.generate_dialogue (Source + notes ──► Gemini text model)
                                       │
                                       ▼
                              list[DialogueChunk]  (~2200 UTF-8 bytes each, split at speaker turns)
                                       │
...
- **Chunk byte budget**: `_MAX_CHUNK_BYTES = 2200` in `llm_summarizer.py`. The TTS prompt prepends a preamble (personalities + optional per-speaker `voice_direction` + scene + director's notes); total must stay below Gemini TTS's ~4000-byte text limit. Measured worst case for the preamble is 1568 bytes (two 160-char voice directions, a 200-char scene, a 50-char `pace`, 11-char names, 125-char personalities, and a long language name, which the header prints twice), so 2200 + 1568 = 3768, about 232 bytes of headroom. The five built-in duos measure 1278 to 1339 bytes in practice (3539 worst case). Note that the `pace` and the language name are both real axes: the built-in `explorer` duo already ships a 50-char `pace`, so an envelope measured with a short `pace` understates the true maximum. Any change to the preamble envelope (longer directions, longer scene) requires re-measuring through `tts_generator._build_tts_prompt` and lowering `_MAX_CHUNK_BYTES` accordingly; `tests/test_tts_generator.py::TestPreambleByteBudget` enforces a 200-byte floor on the headroom for every built-in duo and for both synthetic envelopes. Splits **only at speaker-turn boundaries** (lines starting with `<SpeakerName>:`).
