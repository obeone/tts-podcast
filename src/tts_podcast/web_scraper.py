...
def _extract_from_html(url: str, downloaded: str) -> Source:
    ...
    # Capture outbound links at scrape time.  Plain-text extraction drops all
    # hyperlinks, so the link-following stage would find 0 candidates from
    # full_text.  A second trafilatura pass with include_links=True and
    # output_format="markdown" emits the article body's links as [text](url);
    # collect_document_links puts those first and then falls back to the raw
    # anchors for pages whose body detector under-selects.  full_text stays
    # clean plain text — never polluted with markdown syntax.
    try:
        links_md = trafilatura.extract(downloaded, include_links=True, output_format="markdown") or ""
    except Exception:  # noqa: BLE001
        links_md = ""
    links = collect_document_links(links_md, downloaded, base_url=url)
